...
eth_prices AS (
    ...
    WHERE blockchain = 'ethereum'
      ...
      AND timestamp >= TIMESTAMP '{{start_date}} 00:00:00 UTC'
      AND timestamp < TIMESTAMP '{{end_date}} 00:00:00 UTC'
),
px AS (
    SELECT
    ...
    WHERE p.blockchain = 'ethereum'
      AND p.timestamp >= TIMESTAMP '{{start_date}} 00:00:00 UTC'
      AND p.timestamp < TIMESTAMP '{{end_date}} 00:00:00 UTC'
...
SELECT
    date_add('hour',
             {{bucket_hours}} * CAST(floor(hour(p.ts) / {{bucket_hours}}) AS bigint),
