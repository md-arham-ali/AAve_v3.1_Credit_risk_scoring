...
px AS (   -- hourly USD price for the target assets
    SELECT
    ...
    WHERE blockchain = 'ethereum'
      AND timestamp >= TIMESTAMP '2025-11-01 00:00:00 UTC'
      AND timestamp <  TIMESTAMP '2026-02-01 00:00:00 UTC'
      AND contract_address IN (SELECT asset FROM assets)
),
weth AS (   -- hourly WETH/USD = the ETH and WETH denominator
    SELECT
        CAST(timestamp AS timestamp)   AS ts,
        price                          AS weth_usd
    FROM prices.hour
...
SELECT
    date_add('hour',
             6 * CAST(floor(hour(p.ts) / 6) AS bigint),
             date_trunc('day', p.ts))                  AS time_bucket,
    p.asset,
    MAX(d.decimals)                                    AS decimals,
    AVG(p.price_usd)                                   AS avg_price_usd,
    AVG(p.price_usd / NULLIF(w.weth_usd, 0))           AS avg_price_eth,
    AVG(p.price_usd / NULLIF(w.weth_usd, 0))           AS avg_price_weth,
    COUNT(p.price_usd)                                 AS price_points
FROM px p
LEFT JOIN weth w ON w.ts = p.ts
